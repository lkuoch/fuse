...
export function node<
  ...
  Interfaces extends InterfaceParam<BuilderTypes>[] = InterfaceParam<BuilderTypes>[]
>(
  ...
  const node = builder.loadableNode(opts.name, {
    ...
    async load(
      ids: Array<Key>,
      ctx: { request: Request; params: GraphQLParams } & Record<string, unknown>
    ) {
